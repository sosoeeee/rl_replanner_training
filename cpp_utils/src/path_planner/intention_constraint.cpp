  ...
  float inflated_distance)
{
  ...
  // Compute cone geometry (same as Python implementation)
  float height = depth;
  std::vector<float> height_dirc = robot_direction;

  // Cone center (before inflation)
  std::vector<float> cone_center = {
    cur_pos[0] + depth * height_dirc[0],
    cur_pos[1] + depth * height_dirc[1]
  };

  // Inflate cone center forward
  std::vector<float> inflated_center = {
    cone_center[0] + height_dirc[0] * inflated_distance,
    cone_center[1] + height_dirc[1] * inflated_distance
  };

  // Inflate radius outward
  float phi = (radius > 0) ? std::atan(height / radius) : M_PI / 2.0;
  ...
  base_v0.x = inflated_center[0] + inflated_radius * height_dirc[1];
  base_v0.y = inflated_center[1] - inflated_radius * height_dirc[0];
  base_v1.x = inflated_center[0] - inflated_radius * height_dirc[1];
  base_v1.y = inflated_center[1] + inflated_radius * height_dirc[0];

  // Inflated apex vertices (two points near robot position, pushed backward)
  float inflated_robot_x = cur_pos[0] - height_dirc[0] * inflated_distance;
  float inflated_robot_y = cur_pos[1] - height_dirc[1] * inflated_distance;
  float apex_offset = inflated_distance * std::tan(phi / 2.0);

  Vertex apex_v0, apex_v1;
  apex_v0.x = inflated_robot_x + apex_offset * height_dirc[1];
  apex_v0.y = inflated_robot_y - apex_offset * height_dirc[0];
  apex_v1.x = inflated_robot_x - apex_offset * height_dirc[1];
  apex_v1.y = inflated_robot_y + apex_offset * height_dirc[0];

  // Store vertices in order: apex_v0, base_v0, base_v1, apex_v1 (CCW traversal)
  trapezoid_vertices_.clear();
  trapezoid_vertices_.push_back(apex_v0);
  trapezoid_vertices_.push_back(base_v0);
  trapezoid_vertices_.push_back(base_v1);
  trapezoid_vertices_.push_back(apex_v1);
  ...
  float & min_y, float & max_y) const
{
  ...
  min_x = trapezoid_vertices_[0].x;
  max_x = trapezoid_vertices_[0].x;
  min_y = trapezoid_vertices_[0].y;
  max_y = trapezoid_vertices_[0].y;

  for (size_t i = 1; i < trapezoid_vertices_.size(); ++i) {
    min_x = std::min(min_x, trapezoid_vertices_[i].x);
    max_x = std::max(max_x, trapezoid_vertices_[i].x);
    min_y = std::min(min_y, trapezoid_vertices_[i].y);
    max_y = std::max(max_y, trapezoid_vertices_[i].y);
  }
}
...
bool ConeIntentionConstraint::isRestrictedArea(float x, float y) const
{
  ...
  for (size_t i = 0; i < 4; ++i) {
    const Vertex & v_curr = trapezoid_vertices_[i];
    const Vertex & v_next = trapezoid_vertices_[(i + 1) % 4];
