...
void VoronoiNode::updateProbability(int id) {
    for (auto& adj : adjacent) {
        if (adj.first == id) {
            ...
            int adjacent_size = 1/prob; // Get the size of the adjacent nodes
            for (auto& adj : adjacent) {
                if (adj.second != 0.0f) {
                    adj.second += prob / (adjacent_size - 1); // Distribute the probability of the removed node to the remaining nodes
                }
            }
            break;
        }
